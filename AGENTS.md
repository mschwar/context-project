...
- PyPI-ready packaging: verify name availability (`ctx` was a 2022 supply-chain case; use `ctx-tool` or `ctx-ai` as fallback), update `pyproject.toml` classifiers and URLs, bump `__version__` to `"0.8.0"`.
- GitHub Actions publish workflow (`.github/workflows/publish.yml`): tag `v*` → run tests → build + publish via OIDC trusted publishing (no API token secrets).
