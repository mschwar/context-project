...
- **Structured response schemas** documented in a machine-readable format:
  ...
  ```
  > **Note:** These examples are simplified sketches from early planning. The canonical JSON envelope schema — including the `{status, command, metadata, data, errors, recommended_next}` wrapper and the full per-command `data` field definitions — is defined in `docs/afo/00-conventions.md` and `docs/afo/01-output-broker.md`.
    ...
    "tools": [
      {
        ...
        "output_schema": { "type": "object", "properties": { "status": { "type": "string" }, "dirs_processed": { "type": "integer" }, "tokens_used": { "type": "integer" }, "errors": { "type": "array" } } }
      },
      {
        "name": "ctx_check",
...
- [x] **24.5 Prompt calibration for repo-specific summaries** — prompt templates now push repo-specific nouns, ban generic phrases like `main entry point` / `central hub`, and describe `SKILL.md` / prompt files accurately.
...
- **`CONTEXT.md` filename** — This is the product's strongest discovery mechanism. Any agent that encounters a `CONTEXT.md` in a repo can read it and understand what generated it. The file itself is the advertisement. Consider adding a small footer to generated manifests:
  ```
  <!-- Generated by ctx (https://pypi.org/project/ctx-tool/) — pip install ctx-tool -->
  ```
