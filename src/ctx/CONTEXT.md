---
generated: '2026-03-18T19:37:16Z'
generator: ctx/0.8.0
model: claude-haiku-4-5-20251001
content_hash: sha256:1cb754ca1c8ba5e88ccea5d13c9a1649b97fb076529074253eccf2685da9126d
files: 17
dirs: 1
tokens_total: 17509
---
...
- **server.py** — FastAPI server exposing CONTEXT.md manifests via HTTP endpoints with path traversal protection.
...
- The architecture follows a layered pattern: CLI (cli.py) → API (api.py) → Generator (generator.py) → LLM (llm.py), with supporting utilities for file hashing, git integration, and ignore patterns.
- Configuration is centralized in config.py and supports multiple LLM providers (Anthropic, OpenAI) with cost estimation and connectivity checks.
- Concurrency is managed via lock.py to prevent race conditions during manifest writes; watcher.py enables incremental updates on file changes.
- Manifest format uses YAML frontmatter in CONTEXT.md files, parsed and written by manifest.py; output.py provides structured JSON envelopes for programmatic consumption.
- Language detection and parsing are extensible through lang_parsers subdirectory, supporting extraction of public APIs and structural metadata.
